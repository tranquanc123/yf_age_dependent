...
model {
  ...
  for(ii in 1:N){
    gamma_10[ii] = pow(10.0, gamma[ii]);
  }
  
  //Combine to get the FOI matrix:
  FOI_m = (rep_matrix(gamma_10, 100))';
...
coun_list_study_prior[!(coun_list_study_prior %in% FOI_prior$ISO)] <- "BRA"
sero_list_study = sapply(sero_study_id, function(x) filter(sero_data, STUDY_ID == x)$ISO %>% unique)
match_prior_gamma = match(c(sero_list_study, coun_list_study_prior), FOI_prior$ISO)
prior_gamma_input = FOI_prior[match_prior_gamma,]
...
#initial pars:
init_f <- function(n = 1){
  list(gamma = rnorm(n*N_study,  prior_gamma_input$X1, prior_gamma_input$X2),
...
stan_fit <- stan(
  ...
  warmup = 5e3,          # number of warmup iterations per chain
  iter = 1e4,            # total number of iterations per chain
...
saveRDS(stan_fit, file = paste0("../output/MCMC_output/MCMC_output_1e4_noage_init_rstan.Rdata"))

traceplot(stan_fit, pars = c("gamma[2]", "VE", "lp__"))
